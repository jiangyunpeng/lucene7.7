...
/**
 * SegmentTermsEnum 代表可以遍历Filed的Term
...
final class SegmentTermsEnum extends BaseTermsEnum {
    ...
    final BytesRefBuilder term = new BytesRefBuilder(); //保存查询到的结果
    ...
    public SegmentTermsEnum(FieldReader fr) throws IOException {
        ...
        if (fr.index == null) {
            ...
            fstReader = fr.index.getBytesReader();//返回fastReader，读取的是tip文件内存映射
        }
    ...
    /**
     * ord 表示序号，该方法会复用 arcs 数组中的Arc对象，避免频繁创建Arc新对象
     * 当ord大于等于arcs.length会扩容，arcs默认是1
     */
    private FST.Arc<BytesRef> getArc(int ord) {
        ...
        if (ord >= arcs.length) {
            ...
            //这里为扩容后的数组创建对象，比如之前有2个Arc，扩容到6，那么从2,3,4,5开始
    ...
    /**
     *Pushes a frame we seek'd to,参数：
     * - arc： 我们flow的边，初始是从root开始
     * - frameData：边的output，包含fpSeek、hasTerms、isFloor等信息
     */
    SegmentTermsEnumFrame pushFrame(FST.Arc<BytesRef> arc, BytesRef frameData, int length)
        ...
        //提取文件指针和标志位,右移去掉标志位，得到文件指针
        final long fpSeek = code >>> Lucene90BlockTreeTermsReader.OUTPUT_FLAGS_NUM_BITS;
        final SegmentTermsEnumFrame f = getFrame(1 + currentFrame.ord);//从栈中获取当前帧，会自动扩容
        ...
        if (f.isFloor) { //如果是floor块，读取额外的floor数据
            f.setFloorData(scratchReader, frameData);
        }
        //压入新帧
        pushFrame(arc, fpSeek, length);

        SourceLogger.info(SegmentTermsEnum.class, "pushFrame ord:{}, prefixNum:{}",1 + currentFrame.ord,f.prefix);
        return f;
    }
  ...
  */
    ...
    public boolean seekExact(BytesRef target) throws IOException {
        ...
        SourceLogger.info(SegmentTermsEnum.class, "seekExact()! field:{}, value:{}", fr.fieldInfo.name, new String(target.bytes) );
        // if (DEBUG) {
        ...
        if (currentFrame != staticFrame) {
            ...
            // if (DEBUG) {
               SourceLogger.info(SegmentTermsEnum.class,"  re-use current seek state validIndexPrefix=" + validIndexPrefix);
            // }
            ...
            arc = fr.index.getFirstArc(arcs[0]);//获取虚拟的arc，指向的是root节点
            ...
            // term.length = 0;
            targetUpto = 0;
            BytesRef frameData = Lucene90BlockTreeTermsReader.FST_OUTPUTS.add(output, arc.nextFinalOutput());//返回root的输出值
            currentFrame = pushFrame(arc, frameData, 0);
        }
        ...
        while (targetUpto < target.length) { //按照字符长度迭代
            final int targetLabel = target.bytes[target.offset + targetUpto] & 0xFF;
            SourceLogger.info(this.getClass(),"seekExact begin find arc！ seq:[{}] label:{}/{},follow.target:[{}] ",targetUpto,
                    (char)targetLabel,
                    targetLabel,
                    arc.target());
            //通过FST找到的对应的边，从arc开始查找
            final FST.Arc<BytesRef> nextArc = fr.index.findTargetArc(targetLabel, arc, getArc(1 + targetUpto), fstReader);

            if (nextArc == null) {//已经结束
                ...
                //这里说明已经找到前缀，加载tim文件，读取完整的term
                currentFrame.loadBlock();//加载tim文件
                //从当前block中查找完成term
                ...
                // 说明找到了前缀，比如cat这个term，从'c'开始，如果找到了'c'就继续从'c'这条边开始往后找
                arc = nextArc;
                term.setByteAt(targetUpto, (byte) targetLabel);//这里记录已经找到的字符
                ...
                if (arc.isFinal()) {//如果arc指向了终止节点
                    SourceLogger.info(this.getClass(),"Arc [{}] is final! target:{}",(char)targetLabel,arc.target());
                    //把这条边的output和cur推入stack中，同时更新currentFrame
                    BytesRef frameData = Lucene90BlockTreeTermsReader.FST_OUTPUTS.add(output, arc.nextFinalOutput());
                    currentFrame = pushFrame(arc, frameData, targetUpto);
    ...
    public TermState termState() throws IOException {
        assert !eof;
        currentFrame.decodeMetaData();
        TermState ts = currentFrame.state.clone();//实际是 BlockTermState
