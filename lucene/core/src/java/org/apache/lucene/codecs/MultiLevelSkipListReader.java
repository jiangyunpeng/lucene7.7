...
public abstract class MultiLevelSkipListReader implements Closeable {
    ...
    protected MultiLevelSkipListReader(
        ...
        for (int i = 1; i < maxSkipLevels; i++) {
            ...
            this.skipInterval[i] = this.skipInterval[i - 1] * skipMultiplier;//保存每一层的skip interval
        }
    ...
    public int skipTo(int target) throws IOException {
        ...
        while (level >= 0) {
            ...
            SourceLogger.info(this.getClass(),
                    "advance target=%s, skipDoc[%s]=%s, lastDoc=%s, lastChildPointer=%s",
                    target, level,skipDoc[level], lastDoc, lastChildPointer);

            if (target > skipDoc[level]) {//如果target大于该level中的skipDoc
                if (!loadNextSkip(level)) {//继续读取该level数据
                    continue;
                }
            } else {
                // no more skips on this level, go down one level
                /// 如果当前节点 lastChildPointer 大于下一层的pos，则可以直接跳过lastChildPointer
    ...
    private boolean loadNextSkip(int level) throws IOException {
        ...
        numSkipped[level] += skipInterval[level];//按照每一层skipInterval自增
    ...
    protected void seekChild(int level) throws IOException {
        ...
        //快速更新numSkipped，本层numSkipped等于上层numSkipped-skipInterval
    ...
    private void loadSkipLevels() throws IOException {
        ...
        //doc seek到docTermStartFP+skipOffset位置
        skipStream[0].seek(skipPointer[0]);

        //从顶层向下初始化每次的skipStream，除了0层️
    ...
    protected void setLastSkipData(int level) {
        lastDoc = skipDoc[level];//保存上一个doc
        lastChildPointer = childPointer[level];//保存上一个doc的child位置 ，level-0永远是0
    }
}
